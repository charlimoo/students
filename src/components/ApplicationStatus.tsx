...
interface ApplicationStatusProps {
  ...
  onBack: () => void;
  onNavigate: (page: string, id?: string) => void; // Add this prop
}
// --- Type Definition for API Data ---
// This interface matches the structure from your ApplicationDetailSerializer
interface ApiApplicationDetail {
  ...
  logs: { id: number; actor: { full_name: string } | null; action: string; timestamp: string; }[];
}
    ...
    <div className="flex-1 overflow-auto">
      <div className="container-modern py-8 space-y-8">
        ...
        <div className="space-y-6">
            ...
            <Card><CardHeader><CardTitle className="flex items-center space-x-2"><FileText className="w-5 h-5 text-primary"/><span>Submitted Documents</span></CardTitle></CardHeader><CardContent className="space-y-3">{application.documents.map((doc) => <div key={doc.id} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg"><div className="font-medium">{doc.document_type}</div><a href={doc.file} target="_blank" rel="noreferrer"><Button variant="outline" size="sm">View Document</Button></a></div>)}</CardContent></Card>
            <Card>
              <CardHeader>
                ...
                {application.logs.map((log) => (
                  <div key={log.id} className="flex items-start space-x-4">
                    ...
                    <div className="flex-1">
                      ...
                      {/* --- FIX STARTS HERE --- */}
                      {/* Check if a comment exists and display it in a distinct box */}
                      {log.comment && (
